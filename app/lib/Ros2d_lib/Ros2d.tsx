...
export function rosToCanvas(
  ...
  const x = rosX * mapScale + originX
  const y = originY - rosY * mapScale

  return { x, y }
}


export function canvasToRos(
  x: number,
  y: number,
  mapScale: number,
  originX: number,
  ...
  const rosX = (x - originX) / mapScale
  const rosY = (originY - y) / mapScale
